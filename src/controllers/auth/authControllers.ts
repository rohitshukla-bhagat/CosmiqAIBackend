...
//? Two factor authentication setup handler function
export async function twoFASetupHandler(req: Request, res: Response) {
  ...
  try {
    ...
    const issuer = "CosmiqAI";
    if (!user.email) {
      return res
        .status(401)
        .json({ message: "User email is not found in 2FA authentication" });
    }
...
//? 2 factor authentication verify handler function
export async function twoFAVerifyHandler(req: Request, res: Response) {
  ...
  if (!code) {
    return res.status(400).json({ message: "2FA Code is not found" });
  }
  try {
    const user = await User.findById(authUser.id);
    if (!user) {
    ...
    if (!isValid) {
      return res.status(404).json({ message: "Invalid 2 factor code" });
    }
